  ...
  try {
    ...
    execQuery(addIgvSlots)
      .then((rows) => {
        res
          .status(200)
          .json({ message: "New iGV Project Slot Added Successfully" });
      })
  ...
  try {
    ...
    execQuery(updateIgvSlotQuery)
      .then((rows) => {
        res
          .status(200)
          .json({ message: "iGV Project slot edited successfully" });
      })
      .catch((err) => {
        next(err);
      });
  ...
  try {
    ...
    execQuery(deleteIgvSlotQuery)
      .then((rows) => {
        res
          .status(200)
          .json({ message: "iGV Project Slot Deleted Successfully" });
      })
      .catch((err) => {
        next(err);
      });
