...
router.get(`/members`, (req, res, next) => {
  execQuery(`
    ...
    FROM member as m WHERE m.front_office_id = "oGT" AND m.department_id="CXP";`)
    .then((rows) => {
...
router.post("/applicants", (req, res, next) => {
  try {
    ...
    const insertQuery = `INSERT INTO ogt_applicants (${fields.toString()}) VALUES (${values.toString()})`;
...
router.delete("/applicants/:id", (req, res, next) => {
  const deleteQuery = `DELETE FROM ogt_applicants WHERE id=${req.params.id}`;
...
router.post("/sendESEMail", async (req, res, next) => {
  try {
    ...
    await execQueryWithValues(
      `UPDATE ogt_applicants SET isEseEmailSent = 1 WHERE email = ?`,
      [receiver]
    );
