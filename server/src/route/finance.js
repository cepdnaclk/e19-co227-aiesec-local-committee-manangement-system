...
router.get("/pending-claims", async (req, res, next) => {
    ...
    const queryString = `SELECT ${tables.map(table => generateCountQueryForTable(table.tableName, table.alias)).join(', ')}`;
...
router.get("/totalRevenue", async (req, res, next) => {
    ...
    // Construct the query dynamically
    const queryString = `SELECT ${tables.map(table => generateSumQueryForTable(table.tableName, table.alias)).join(', ')}`;
...
router.get("/claimDetails", async (req, res, next) => {
    ...
    const igvQueryString = `SELECT appId, team FROM igv_application WHERE claimStatus = FALSE`;
    const ogvQueryString = `SELECT id, firstName FROM ogv_applicants WHERE claimStatus = FALSE`;
