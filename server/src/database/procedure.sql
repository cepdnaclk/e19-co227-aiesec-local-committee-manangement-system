...
BEGIN
    SELECT 
        ...
        a.hostMc,
        a.hostLc,
        DATE_FORMAT(a.acceptedStartDate, "%Y-%m-%d") as acceptedStartDate,
        DATE_FORMAT(a.acceptanceDate, "%Y-%m-%d") as acceptanceDate,
        a.acceptedNotes,
        DATE_FORMAT(a.approvedDate, "%Y-%m-%d") as approvedDate,
        DATE_FORMAT(a.paymentDate, "%Y-%m-%d") as paymentDate,
        a.paymentAmount,
        a.proofLink,
        a.approvedNotes,
        DATE_FORMAT(a.realizedStartDate, "%Y-%m-%d") as realizedStartDate,
        a.realizedNotes,
        DATE_FORMAT(a.finishedDate, "%Y-%m-%d") as finishedDate,
        DATE_FORMAT(a.completedDate, "%Y-%m-%d") as completedDate,
        a.approvalBreakNote,
        a.realizationBreakNote,
        GROUP_CONCAT(e.mailtemplateId) AS sentMailTemplateIds  -- Collect all the sent mailtemplateIds
    FROM 
        ogv_applicants AS a
    LEFT JOIN 
        email_to_ogv_applicants AS e ON a.id = e.applicantId
    WHERE 
        a.id = applicant_id
    GROUP BY 
        a.id;  -- Group by applicantId to get a single row for each applicant
END;
